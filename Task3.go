...
func (*StudentList) Print_Method(SL *StudentList) {
	...
	for no := 0; no < total; no++ {
		...
		fmt.Println("Student ", no+1, " Roll# : ", SL.list[no].rollnumber)
		fmt.Println("Student ", no+1, " Name of Student: ", SL.list[no].name)
...
func main() {
	...
	student.CreateStudent(19345, "Manan", "Islamaad")
	student.CreateStudent(19873, "Ali", "Lahore")
